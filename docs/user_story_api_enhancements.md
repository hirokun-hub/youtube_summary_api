...
| error_code | 意味 | success |
|-----------|------|---------|
| `null` | エラーなし | `true` |
| `INVALID_URL` | YouTube URLとして認識できない | `false` |
| `VIDEO_NOT_FOUND` | 動画が存在しない・非公開・削除済み | `false` |
| `TRANSCRIPT_NOT_FOUND` | 字幕が見つからない（メタデータは取得済み） | `false` |
| `RATE_LIMITED` | YouTubeへのリクエスト過多 | `false` |
| `METADATA_FAILED` | yt-dlpによるメタデータ取得に失敗（oEmbedにフォールバック） | `true`（字幕が取れていれば） |
| `INTERNAL_ERROR` | 予期せぬエラー | `false` |
