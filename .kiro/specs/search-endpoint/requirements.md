...
**既存の `POST /api/v1/summary` の既存フィールド・既存挙動は完全に維持し、新規フィールドの追加のみを行う。**

- 既存フィールド（`success`, `status`, `message`, `transcript` 等 22 項目）は **名前・型・値・形式すべて不変**
...
- 既存の HTTP ステータス（常に 200）も不変
- **新規追加（既存クライアント影響なし）**: レスポンスに `quota` オブジェクトを追加。iPhone ショートカットは未知フィールドを単に無視するため後方互換性に影響しない
...
**As a** AI エージェント  
**I want** **認証通過後の業務レスポンス（200/429/503/500）すべて**に「今日あとどれだけ検索できるか」の目安と、**次のリセットまでの残り秒数**が含まれる  
...
> 注: 認証エラー（401）とリクエスト構造違反（422）には `quota` を含めない。認証前にクォータを開示する必要がなく、また 422 は FastAPI/Pydantic 標準の `detail[]` 形式で返すため。
...
**As a** AI エージェント  
**I want** レート制限で拒否された時、そのルール（例: `"max 10 requests per 60 seconds"`）と `retry_after` 秒数がエラーメッセージ本文に含まれる  
...
下記サンプルでは `results` 配列に 1 件のみ示すが、実際は **`returned_count` の値ぶん**（最大 50 件、YouTube 側のヒット数次第で減る）の `SearchResult` オブジェクトが格納される。サンプル JSON はそのまま `json.loads` できる構造（コメント不使用）。
...
`/search` は LLM Tool SDK との相性を優先して **標準 HTTP ステータスを返す**（既存 `/summary` は 200 固定を維持。詳細は TC-9 を参照）。

レスポンスボディの形式は次の2種類:

- **基本形式**（401 / 429 / 503 / 500）: 成功時と同じ `success` / `error_code` / `message` を含む。`quota` フィールドは **認証通過後のレスポンス（429 / 503 / 500）にのみ含み、401 には含めない**（認証前にクォータを開示しない方針）
- **FastAPI/Pydantic 標準形式**（422 のみ）: `{"detail": [...]}` 形式を維持。`quota` も `success` / `error_code` も含まない

エラー時のステータス正規化方針:

- YouTube が 403 `quotaExceeded` を返した場合 → クライアントには **HTTP 429** + `Retry-After` で返す（`QUOTA_EXCEEDED`）
- YouTube が 429 を返した場合 → クライアントには **HTTP 503** で返す（`RATE_LIMITED`）
- 上流の HTTP コードをそのまま透過させない（クライアントが意味解釈に迷わないため）
...
```json
{
  "success": false,
  "status": "error",
  "message": "Search rate limit exceeded: more than 10 requests in the last 60 seconds. Rule: max 10 requests per 60 seconds. Retry after 12 seconds.",
...
```json
{
  "success": false,
  "status": "error",
  "message": "YouTube Data API daily quota (10000 units) exhausted. Resets in 32400 seconds (at 2026-04-26T16:00:00+09:00 JST).",
...
#### 認証エラー（`HTTP 401 Unauthorized`、`quota` フィールドなし）
...
認証通過前のため `quota` は含まれない（クォータを認証前に開示しない方針）。

#### リクエスト構造違反（`HTTP 422 Unprocessable Entity`）

FastAPI / Pydantic の標準バリデーションエラー形式（`{"detail": [...]}`）をそのまま返す。`q` 未指定、`order` が列挙外、`published_after` が ISO 8601 不正などが該当。`quota` は含まれない。

#### YouTube 側の一時制限（`HTTP 503 Service Unavailable` + `Retry-After`）

IP ブロック・短期スロットル・YouTube が 429 を返したケースなど。`RATE_LIMITED` エラーコードで返す（上流の 429 を 503 に正規化）。
...
| error_code | 発生条件 | HTTP status | 追加ヘッダ | レスポンスボディ形式 | `quota` フィールド |
|---|---|---|---|---|---|
| `null` | 成功 | 200 | — | 基本形式 | あり |
| `UNAUTHORIZED` | `X-API-KEY` ヘッダが不正または未設定 | 401 | — | 基本形式 | **なし** |
| （Pydantic 標準） | リクエストボディのスキーマ違反 | 422 | — | `{"detail": [...]}` | **なし** |
| `CLIENT_RATE_LIMITED` | 自サーバのレート制限超過（直近60秒10回） | 429 | `Retry-After: <秒数>` | 基本形式 | あり |
| `QUOTA_EXCEEDED`（**新設**） | 内部カウンタが日次上限到達 or YouTube が 403 quotaExceeded を返した | **429**（403 ではない） | `Retry-After: <秒数>` | 基本形式 | あり |
| `RATE_LIMITED` | YouTube 側の一時的制限（IP ブロック、YouTube 429 の正規化） | **503**（429 ではない） | `Retry-After: <秒数>`（入手可能な場合） | 基本形式 | あり |
| `INTERNAL_ERROR` | 予期せぬエラー | 500 | — | 基本形式 | あり |

**ステータス正規化のルール**:

- YouTube の 403 `quotaExceeded` → API としては **HTTP 429** に統一（理由: クライアントは「待てば回復する」を `Retry-After` で受け取るため、403 より 429 の方が再試行可能性を表現できる）
- YouTube の 429（短期レート制限）→ API としては **HTTP 503** に統一（理由: 自サーバ側の制限である `CLIENT_RATE_LIMITED`（429）と区別するため）
...
- 上限超過時:
  ...
  - `error_code = CLIENT_RATE_LIMITED`
- エラーメッセージには **ルール本文（"max 10 requests per 60 seconds"）と retry_after 秒数** を必ず含める（AI の学習容易性）
...
5. `channels.list(id=<ids>, part=snippet,statistics)` を 1 回呼ぶ（1 unit）
   - **`snippet` を含める理由**: `channel_created_at`（チャンネル作成日）は `snippet.publishedAt` から取得する
   - **`statistics` の理由**: `subscriberCount`, `videoCount`, `viewCount` を取得する
   - 既存の `app/core/constants.py` の定数 `YOUTUBE_API_V3_CHANNELS_PART = "statistics"` を **`"snippet,statistics"`** に変更する必要あり
6. 3 つのレスポンスを結合して `results[]` を組み立て
7. 派生値（`like_view_ratio`, `comment_view_ratio`, `channel_avg_views`）を計算（分母 0 時は `null`）
...
- **Python 3.12**（既存の Docker イメージ `python:3.12-slim` 準拠）
- `itertools.batched`（Python 3.12 標準）でバッチ分割を行う

### 新規依存

- 追加ライブラリなし（既存の `requests` で YouTube Data API v3 を呼べる）
- Python 標準の `sqlite3` と `zoneinfo` を利用
...
- `app/core/constants.py`:
  - `ERROR_QUOTA_EXCEEDED` / `ERROR_UNAUTHORIZED` 追加
  - search 用レート制限定数（`SEARCH_RATE_LIMIT_WINDOW_SECONDS=60`, `SEARCH_RATE_LIMIT_MAX_REQUESTS=10`）追加
  - `MSG_QUOTA_EXCEEDED` / `MSG_UNAUTHORIZED` / search 用メッセージ追加
  - **`YOUTUBE_API_V3_CHANNELS_PART = "statistics"` を `"snippet,statistics"` に変更**（`channel_created_at` 取得のため、TC-7/FR-10 参照）
- `app/models/schemas.py`: `SummaryResponse` に `quota` フィールド（Optional）を追加。新規モデルは `X | None` 統一・`ConfigDict(frozen=True, extra="forbid")`
- `app/routers/summary.py`: quota 集計を挟む（既存挙動・既存フィールドは不変）
- `app/services/youtube.py`: videos.list / channels.list 呼び出し後に `consumed_units` を加算（`channels.list` の part 変更に伴う応答パース修正含む）
...
| YouTube 日次クォータ枯渇 | **429**（403 ではない） | `QUOTA_EXCEEDED` |
| YouTube 一時制限 | **503**（429 ではない） | `RATE_LIMITED` |
| 内部エラー | 500 | `INTERNAL_ERROR` |

- 429 / 503 には **`Retry-After` ヘッダ** を付与
- 上流（YouTube）の HTTP コードはそのまま透過させず、本表のとおり **API として正規化** する
- レスポンスボディは基本形式（`success`, `error_code`, `message`, `quota`）を維持。ただし **422 のみ FastAPI/Pydantic 標準の `{"detail": [...]}` 形式**、**401/422 には `quota` を含めない**
...
9. ✅ 直近 60 秒に 11 回目を叩くと **HTTP 429** + `Retry-After: <秒数>` ヘッダ + `error_code=CLIENT_RATE_LIMITED` が返り、ルール本文と `retry_after` 秒数がメッセージに含まれる
