...
- **共通形式**（401 / 429 / 503 / 500）: 成功時と同じ `success` / `error_code` / `message` / `quota` 形式を保つ
- **FastAPI/Pydantic 標準形式**（422 のみ）: `{"detail": [...]}` 形式を維持。これはリクエスト構造違反のため、`quota` フィールドは付与しない（認証通過済みかつクォータ消費前のため）
