...
- **基本エラー形式**（401 / 429 / 503 / 500）: 成功時と同じ `success` / `error_code` / `message` を含む。`quota` フィールドは **認証通過後のレスポンス（429 / 503 / 500）にのみ含み、401 には含めない**（認証前にクォータを開示しない方針）
...
|---|---|---|---|---|---|
| `null` | 成功 | 200 | — | 共通形式 | あり |
| `UNAUTHORIZED` | `X-API-KEY` ヘッダが不正または未設定 | 401 | — | 共通形式 | **なし** |
| （Pydantic 標準） | リクエストボディのスキーマ違反 | 422 | — | `{"detail": [...]}` | **なし** |
| `CLIENT_RATE_LIMITED` | 自サーバのレート制限超過（直近1分10回） | 429 | `Retry-After: <秒数>` | 共通形式 | あり |
| `QUOTA_EXCEEDED`（**新設**） | 内部カウンタが日次上限到達 or YouTube が 403 quotaExceeded を返した | **429**（403 ではない） | `Retry-After: <秒数>` | 共通形式 | あり |
| `RATE_LIMITED` | YouTube 側の一時的制限（IP ブロック、YouTube 429 の正規化） | **503**（429 ではない） | `Retry-After: <秒数>`（入手可能な場合） | 共通形式 | あり |
| `INTERNAL_ERROR` | 予期せぬエラー | 500 | — | 共通形式 | あり |
...
- レスポンスボディは共通形式（`success`, `error_code`, `message`, `quota`）を維持。ただし **422 のみ FastAPI/Pydantic 標準の `{"detail": [...]}` 形式**、**401/422 には `quota` を含めない**
