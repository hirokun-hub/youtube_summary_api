...
| youtube-transcript-api | `YouTubeTranscriptApi` クラス | `app.services.youtube.YouTubeTranscriptApi` | クラスをモックし、インスタンスの `list()` / `fetch()` の戻り値を設定 |
...
#### youtube-transcript-api のモック（v1.2.x対応）
...
- **youtube-transcript-api成功レスポンス**: `FetchedTranscript` 互換のモック（`.to_raw_data()` で `list[dict]` を返す）
- **各種失敗レスポンス**: `yt_dlp.utils.DownloadError`, `NoTranscriptFound` 等の例外

### 2.4 環境変数のモック

`main.py` がモジュールレベルで `load_dotenv()` を実行するため、テスト時は以下で対応:
...
| # | テストケース | 検証内容 |
|---|------------|---------|
| S-1 | 全フィールドが定義されていること | SummaryResponse に既存5フィールド + 新規16フィールドが存在する |
...
### 3.2 test_youtube_service.py — サービス層の単体テスト（13ケース）
...
| Y-6 | 異常系: レート制限 | YouTubeRequestFailed | success=False, error_code="RATE_LIMITED" |
...
| Y-9 | transcript_language と is_generated の取得 | transcript成功（list→find_transcript→fetch） | 言語コードと自動生成フラグが正しく設定される |
...
| Y-13 | error_code 6種の全カバレッジ | 各例外パターン | INVALID_URL, VIDEO_NOT_FOUND, TRANSCRIPT_NOT_FOUND, RATE_LIMITED, METADATA_FAILED, INTERNAL_ERROR |
...
Phase 3: サービス層テスト → 実装
  → test_youtube_service.py（Y-1〜Y-13）を書く → RED
  → youtube.py に yt-dlp取得関数 + 新transcript-api対応 + エラーハンドリングを実装 → GREEN
  → リファクタリング
